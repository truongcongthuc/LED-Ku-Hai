...
import { SW, SW_HISTORY, SW_HISTORY_BY_INDICATOR } from '../../config-app'
...
const OneStationNmAutoFetch = () => {
    ...
    const fetchListStation = async () => {
        try {
            ...
            // const res = await getApi('http://localhost:3002/result', '')
            const [rawDataMain, rawDataMerge] = await Promise.all([
                getApi('http://localhost:3002/result', ''),
                getApi('https://server-source-led.onrender.com/getsingledata', '')
            ])
            const res = rawDataMain
            Object.keys(rawDataMerge.data).forEach(key => {
                if (res.data.data.data[key]) {
                    res.data.data.data[key].value = rawDataMerge.data[key].value;
                }
            })
            if (res.data.success) {
    ...
    const fetchStationById = async (id) => {
        ...
        try {
            // http://localhost:3001/result
            // const res = await getApi('http://localhost:3002/result', '') //fetch dữ liệu thực khi lần đầu render
            const [rawDataMain, rawDataMerge] = await Promise.all([
                getApi('http://localhost:3002/result', ''),
                getApi('https://server-source-led.onrender.com/getsingledata', '')
            ])
    ...
    const fetchDataHistory = async (id) => {
        ...
        try {
            // const res = await getApi('http://localhost:3003/result', '') //fetch theo thời gian trôi qua
            const [rawDataMain, rawDataMerge] = await Promise.all([
                getApi('http://localhost:3003/result'),
                getApi('https://server-source-led.onrender.com/getmultidata')
            ])
            // format data
            Object.keys(rawDataMerge.data).forEach((item) => {
                delete Object.assign(rawDataMerge.data[item], { ['series']: rawDataMerge.data[item]['values'] })['values']

                rawDataMerge.data[item]['valMin'] = Math.min(...rawDataMerge.data[item].series)
                rawDataMerge.data[item]['valMax'] = Math.max(...rawDataMerge.data[item].series)
            })

            console.log(rawDataMain.data.data, rawDataMerge.data)

            // merge data
            const res = rawDataMain
            Object.keys(rawDataMerge.data).forEach(item => {
                if (res.data.data[item]) {
                    const itemKeys = Object.keys(rawDataMerge.data[item]).filter(key => res.data.data[item][key]);
                    itemKeys.forEach(key => {
                        res.data.data[item][key] = rawDataMerge.data[item][key];
                    });
                }
            });
            if (res.data.success) {
